 ...
 */
public class RainCommand extends PlayerCommand {


    public RainCommand(CelestitePlugin plugin) {
        super(plugin);
    }

    @Override
    public void execute(Player player, CommandContext commandContext) throws CommandException {
        player.getWorld().setWeather(Weathers.RAIN);
        plugin.getMessenger().sendMessage(player, Levels.SUCCESS, CelestiteMessages.WEATHER_CHANGED, "Rain");
    }
}
