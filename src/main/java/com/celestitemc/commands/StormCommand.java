 ...
 */
public class StormCommand extends PlayerCommand {


    public StormCommand(CelestitePlugin plugin) {
        super(plugin);
    }

    @Override
    public void execute(Player player, CommandContext commandContext) throws CommandException {
        player.getWorld().setWeather(Weathers.THUNDER_STORM);
        plugin.getMessenger().sendMessage(player, Levels.SUCCESS, CelestiteMessages.WEATHER_CHANGED, "Thunder Storm");
    }
}
